    ...
    for (int i = 0; i < max_sim_threads; ++i)
    {
        if (_handle_flags[i].is_unused())
        {
            if (_handle_flags[i].reserve())
            {
                ...
                while ( tempi <= i)
                { _last_handle_id.compare_exchange_weak(tempi, i+1,
...
estrat_sync<P>::local_data_type::local_data_type(P& parent,
    ...
    : _parent(parent), _global(parent._global_exclusion), _worker_strat(wstrat),
      _id(_global.register_handle()),
      _own_flags(_global._handle_flags[_id])
{ }
...
estrat_sync<P>::local_data_type::get_table()
{
    ...
    // from the pool thread !but only if the growing step has been completed in the mean time!
    // growable_table_type* should_be_null = nullptr;
    // if (! _own_flags.compare_exchange_strong(should_be_null, temp,
    //                                        std::memory_order_acq_rel))
    // { help_grow(0, false); return get_table(); }
...
template <class P>
void
estrat_sync<P>::local_data_type::grow(int version)
{
    ...
    if (epoch >= version
        || !_global._epoch.compare_exchange_strong(epoch, version, std::memory_order_acq_rel))
    {
    ...
    dtm::if_debug("Error: _next_table pointer != nullptr in grow()", should_be_null);

    // _protect.store(mark::mark<growing_flag>(temp), std::memory_order_release);
...
template <class P>
void
estrat_sync<P>::local_data_type::help_grow([[maybe_unused]]int version, bool external)
{
    dtm::if_debug("in help_grow, got here from external (how?)", external);
    // wait till migration is safe

    _worker_strat.execute_migration(*this,
                                    _global._epoch.load(std::memory_order_acquire));
...
template <class P>
void
estrat_sync<P>::local_data_type::wait_for_table_op(growable_table_type* current)
{

    auto end = _global._last_handle_id.load(std::memory_order_acquire);
    for (int i = 0; i < end; ++i)
    {
        auto temp = _global._handle_flags[i].ops_protect.load(std::memory_order_acquire);
        while (temp == current)
        {
            temp = _global._handle_flags[i].ops_protect.load(std::memory_order_acquire);
...
estrat_sync<P>::local_data_type::wait_for_migration()
{
    ...
    for (int i = 0; i < end; ++i)
    {
        auto temp = _global._handle_flags[i].mig_protect.load(std::memory_order_acquire);
        while (temp != nullptr)
        {
            temp = _global._handle_flags[i].mig_protect.load(std::memory_order_acquire);
            // if (temp == nullptr) break;
            // if (temp == unused_flags) break;
            // if (mark::is_marked<growing_flag>(temp)) break;
        }
