...
template <class Key, class Data> class seq_complex_slot
{
    struct ptr_split
    {
  ...
  public:
    ...
    static std::string name() { return "seq_complex_slot"; }
};
...
template <class K, class D>
seq_complex_slot<K, D>::slot_type::slot_type(const value_type& pair,
                                             size_t            hash)
    : _mfptr(ptr_union{0})
{
    auto ptr = allocate();
...
template <class K, class D>
seq_complex_slot<K, D>::slot_type::slot_type(ptr_union source) : _mfptr(source)
{
}
...
template <class K, class D>
void seq_complex_slot<K, D>::slot_type::set_fingerprint(size_t hash)
{
    _mfptr.split.fingerprint = seq_complex_slot::fingerprint(hash);
}
...
// *** state *******************************************************************
template <class K, class D>
bool seq_complex_slot<K, D>::slot_type::is_empty() const
{
    return _mfptr.full == 0;
}

template <class K, class D>
bool seq_complex_slot<K, D>::slot_type::is_deleted() const
{
    return _mfptr.full == seq_complex_slot::get_deleted()._mfptr.full;
}

template <class K, class D>
bool seq_complex_slot<K, D>::slot_type::is_marked() const
{
    return false;
}

template <class K, class D>
bool seq_complex_slot<K, D>::slot_type::compare_key(const key_type& k,
                                                    size_t          hash) const
{
...
template <class K, class D>
bool seq_complex_slot<K, D>::slot_type::operator==(const slot_type& r) const
{
...
template <class K, class D>
seq_complex_slot<K, D>::atomic_slot_type::atomic_slot_type(
...
seq_complex_slot<K, D>::atomic_slot_type::operator=(
    const atomic_slot_type& source)
{
...
template <class K, class D>
void seq_complex_slot<K, D>::atomic_slot_type::non_atomic_set(
    const slot_type& source)
{
    _sptr = source._mfptr.full;
}
...
template <class K, class D>
bool seq_complex_slot<K, D>::atomic_slot_type::cas(
    [[maybe_unused]] slot_type& expected, const slot_type& goal)
{
    non_atomic_set(goal);
    return true;
}
...
template <class K, class D>
bool seq_complex_slot<K, D>::atomic_slot_type::atomic_delete(
    slot_type& expected)
{
...
template <class K, class D>
bool seq_complex_slot<K, D>::atomic_slot_type::atomic_mark(slot_type& expected)
{
    return true;
}
...
template <class F, class... Types>
std::pair<typename seq_complex_slot<K, D>::mapped_type, bool>
seq_complex_slot<K, D>::atomic_slot_type::non_atomic_update(F f,
                                                            Types&&... args)
{
    auto key_value = load().get_pointer();
    auto result    = f(key_value->second, std::forward<Types>(args)...);
