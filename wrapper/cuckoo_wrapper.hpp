...
template <class Key, class Data, class Hasher, class Alloc> class cuckoo_wrapper
{
  private:
  ...
  public:
    ...
    cuckoo_wrapper(cuckoo_wrapper&& rhs)
    ...
    template <class... Args> inline emplace_return_type emplace(Args&&... args);
...
class cuckoo_config
{
  public:
    ...
    static_assert(is_viable,
                  "libcuckoo wrapper does not support the chosen flags");

    using table_type =
        cuckoo_wrapper<key_type, mapped_type, hash_fct_type, allocator_type>;
...
{
    bool res = hash.update_fn(k, [k, &f, &args...](mapped_type& d) {
        f(d, std::forward<Types>(args)...);
    });
...
template <class F, class... Types>
typename cuckoo_wrapper<K, D, HF, AL>::insert_return_type
cuckoo_wrapper<K, D, HF, AL>::insert_or_update(const key_type&    k,
                                               ...
                                               Types&&... args)
{
    ...
    hash.upsert(
        k,
        [&f, &args...](mapped_type& v) { f(v, std::forward<Types>(args)...); },
        d);
...
template <class F, class... Types>
typename cuckoo_wrapper<K, D, HF, AL>::insert_return_type
cuckoo_wrapper<K, D, HF, AL>::update_unsafe(const key_type& k, F f,
                                            Types&&... args)
{
...
template <class K, class D, class HF, class AL>
size_t cuckoo_wrapper<K, D, HF, AL>::erase(const key_type& k)
{
    return hash.erase(k);
}
